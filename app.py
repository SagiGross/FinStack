...
================================================
INSTALL:  py -m pip install fastapi uvicorn pyjwt passlib[bcrypt]
RUN:      py app.py
"""
...
def init_db():
    with get_db() as db:
        ...
        CREATE TABLE IF NOT EXISTS employees (
            ...
            gross REAL NOT NULL, hire_date TEXT NOT NULL, term_date TEXT, is_ghost INTEGER DEFAULT 0
        );
...
def seed_db():
    with get_db() as db:
        ...
        sales_hash = bcrypt.hash("sales123")
        db.execute("INSERT OR IGNORE INTO users VALUES (?,?,?,?,?,?)", ("u-admin","admin",admin_hash,"admin","Admin Manager","G&A"))
        db.execute("INSERT OR IGNORE INTO users VALUES (?,?,?,?,?,?)", ("u-eduardo","eduardo",sales_hash,"sales","Eduardo Borotchin","S&M"))
        db.execute("INSERT OR IGNORE INTO users VALUES (?,?,?,?,?,?)", ("u-elad","elad",sales_hash,"sales","Elad Lev","S&M"))
        db.execute("INSERT OR IGNORE INTO users VALUES (?,?,?,?,?,?)", ("u-tiki","tiki",sales_hash,"sales","Tiki Tavero","S&M"))
        ...
        C = lambda *a: a + (now, now)
        cons = [
            C("c01","Ministry of Defence","Indonesia","Government",2026,1,"2026-01-01",800000,60,"03-2026",0,0.75,None,"Demo",now,"u-eduardo"),
            C("c02","Ministry of Defence","Indonesia","Government",2026,1,"2026-01-01",150000,120,"05-2026",0,0.75,None,"Email Sent",now,"u-eduardo"),
            C("c03","Serbia","Serbia","Government",2025,10,"2025-10-01",90000,240,"05-2026",0,0.5,None,"Negotiation",now,"u-elad"),
            C("c04","Practical Cyber Academy","Singapore","Academy",2026,2,"2026-02-01",155000,240,"09-2026",0,0.75,None,"Demo",now,"u-tiki"),
            C("c06","Elta","Israel","Enterprise",2025,10,"2025-10-01",5000,180,"03-2026",0,1.0,3483.87,"Signed",now,"u-elad"),
            C("c07","Elta","Israel","Enterprise",2025,10,"2025-10-01",900000,270,"06-2026",0,0.5,None,"Negotiation",now,"u-eduardo"),
            C("c11","Schools","Israel","Academy",2025,10,"2025-10-01",15000,120,"01-2026",0,1.0,11231.94,"Signed",now,"u-tiki"),
            C("c13","Improvate","Israel","Academy",2026,5,"2026-05-01",150000,60,"06-2026",1,1.0,12108.06,"Signed",now,"u-eduardo"),
            C("c16","Bank of Israel","Israel","Government",2026,1,"2026-01-01",15000,45,"02-2026",0,1.0,None,"Signed",now,"u-elad"),
            C("c18","DSA","Cyprus","Government",2025,10,"2025-10-01",56000,210,"04-2026",0,0.5,35404.52,"Demo",now,"u-tiki"),
            C("c23","MAG","Nigeria","Enterprise",2026,1,"2026-01-01",30000,45,"02-2026",0,1.0,22800,"Signed",now,"u-eduardo"),
            C("c25","Technion","Israel","Academy",2026,1,"2026-01-01",42000,270,"09-2026",1,1.0,3870.97,"Negotiation",now,"u-tiki"),
            C("c28","EU Funding","Greece","Government",2026,1,"2026-01-01",300000,270,"09-2026",0,0.25,None,"Initial Contact",now,"u-elad"),
            C("c31","Abu Dhabi","UAE","Government",2026,1,"2026-01-01",300000,270,"09-2026",0,0.75,None,"Demo",now,"u-eduardo"),
            C("c39","Gabon","Gabon","Government",2026,3,"2026-03-01",450000,120,"06-2026",0,0.5,None,"Email Sent",now,"u-tiki"),
            C("c40","KSV/023","Kosovo","Government",2026,1,"2026-01-01",60000,60,"03-2026",0,1.0,None,"Signed",now,"u-elad"),
            C("c42","Migdal","Israel","Enterprise",2026,1,"2026-01-01",20322.58,45,"02-2026",0,1.0,None,"Negotiation",now,"u-eduardo"),
            C("c48","Ivory Coast","Ivory Coast","Government",2026,1,"2026-01-01",80000,60,"03-2026",0,0.75,None,"Demo",now,"u-tiki"),
        ]
        for c in cons:
            db.execute("INSERT OR IGNORE INTO contracts (id,client,country,industry,yr,sm,sd,val,dso,pm,monthly,chance,ap,stage,stage_updated_at,salesperson_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                       c + ('[]',))
...
def login(body: LoginReq):
    ...
    return {"token": token, "role": u["role"], "name": u["display_name"], "userId": u["id"]}
...
def load_salespeople():
    with get_db() as db:
        return [dict(r) for r in db.execute("SELECT id,display_name,username FROM users WHERE role='sales' OR dept='S&M'").fetchall()]
...
def log_audit(entity_type, entity_id, field, old_val, new_val, user_id):
    with get_db() as db:
        db.execute("INSERT INTO audit_log (entity_type,entity_id,field,old_value,new_value,changed_by,changed_at) VALUES (?,?,?,?,?,?,?)",
                   (entity_type, entity_id, field, str(old_val), str(new_val), user_id, datetime.utcnow().isoformat()))
...
def compute(delay=0, ghosts=0, gs=15000, w=True):
    ...
    for mi,ml in enumerate(ML):
        ...
        for e in employees:
            ...
            if md>=h and (not t or md<=t): lab[e["dept"]]+=e["gross"]*ECF
        vd={"R&D":0,"S&M":0,"G&A":0,"Training":0}; vc=vo=0
        for x in expenses:
            a=x["amounts"].get(ml,0); vd[x["dept"]]+=a
            if x["is_cogs"]: vc+=a
            else: vo+=a
    ...
    for c in contracts:
        if c["ap"] and c["ap"]>0:
            ...
            if idx is not None: data[idx]["cashIn"]+=c["ap"]; continue
        v=c["val"]
        if w: v=v*c["chance"] if c["chance"] is not None else 0
        sd=date.fromisoformat(c["sd"])
        if c["monthly"]:
...
def get_data(weighted:bool=True, delay:int=0, ghosts:int=0, salary:float=15000):
    ...
    es=[{"id":e["id"],"name":e["name"],"dept":e["dept"],"gross":e["gross"],
         "laborCost":round(e["gross"]*ECF,2),"hireDate":e["hire_date"],"termDate":e["term_date"]} for e in employees]
    xs=[{"id":x["id"],"dept":x["dept"],"vendor":x["vendor"],"subCat":x["sub_cat"],
         "isCogs":bool(x["is_cogs"]),"amount":round(sum(x["amounts"].values())/max(len(x["amounts"]),1),2)} for x in expenses]
    sp=load_salespeople()
...
def sales_analytics(weighted:bool=True):
    ...
    by_type={}; by_country={}; total=0
    for c in contracts:
        v=c["val"]
        if weighted: v=v*c["chance"] if c["chance"] is not None else 0
        ind=c["industry"] or "Other"; co=c["country"] or "Unknown"
        by_type.setdefault(ind,{"total":0,"count":0})
        by_type[ind]["total"]+=v; by_type[ind]["count"]+=1
        by_country.setdefault(co,{"total":0,"count":0})
        by_country[co]["total"]+=v; by_country[co]["count"]+=1
        total+=v
    for k in by_type: by_type[k]["total"]=round(by_type[k]["total"],2); by_type[k]["pct"]=round(by_type[k]["total"]/total*100,1) if total>0 else 0
    for k in by_country: by_country[k]["total"]=round(by_country[k]["total"],2); by_country[k]["pct"]=round(by_country[k]["total"]/total*100,1) if total>0 else 0
    top_type=max(by_type,key=lambda k:by_type[k]["total"]) if by_type else None
    top_country=max(by_country,key=lambda k:by_country[k]["total"]) if by_country else None
    # Stage distribution
    stages_dist={s:0 for s in STAGES}
    for c in contracts: stages_dist[c["stage"]]=stages_dist.get(c["stage"],0)+1
    return {"total":round(total,2),"byType":by_type,
...
def add_contract(b:ContractIn, user=Depends(get_current_user)):
    ...
    now=datetime.utcnow().isoformat()
    sp_id = b.salespersonId or (user["sub"] if user["role"]=="sales" else None)
    with get_db() as db:
        db.execute("INSERT INTO contracts (id,client,country,industry,yr,sm,sd,val,dso,pm,monthly,chance,stage,stage_updated_at,salesperson_id,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                   (cid,b.client,b.country,b.industry,sd.year,sd.month,b.signingDate,b.value,b.dso,pm,int(b.monthly),b.chance,"Initial Contact",now,sp_id,"[]",now,now))
...
def upd_employee(eid:str, b:BaseModel, user=Depends(require_admin)):
    d = b.dict(exclude_unset=True) if hasattr(b,'dict') else {}
    # Simplified: accept any fields
    return {"ok":True}
...
def add_expense(b:ExpenseIn, user=Depends(require_admin)):
    xid=str(uuid4())[:8]
    with get_db() as db:
        db.execute("INSERT INTO expenses (id,dept,vendor,sub_cat,is_cogs,amounts) VALUES (?,?,?,?,?,?)",
                   (xid,b.department,b.vendor,b.subCategory,int(b.isCogs),json.dumps({m:b.monthlyAmount for m in ML})))
